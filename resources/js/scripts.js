  ...
  function handleAllSandboxesDeleted() {
    ...
    const interval = setInterval(() => {
      ...
      if (seconds <= 0) {
        ...
        handleSandboxSelection(); // Go back to the Sandbox agreement/check page to create a new one
      }
    }, 1000);
  }
...
/**
 * Displays a full-page confirmation screen for deleting a course.
 * This seems like a standalone delete confirmation, potentially for a different workflow
 * than the table-based actions in `handleSandboxExistsPage`.
 * @param {string} courseID - The ID of the course to be deleted.
 * @param {string} accessToken - User's access token.
 * @param {string} courseName - Name of the course for the confirmation message.
 */
function showDeleteConfirmation(courseID, accessToken, courseName) {
    ...
    const header = document.createElement('h2');
    header.textContent = 'Confirm Delete Sandbox Course';
    processContainer.appendChild(header);

    const message = document.createElement('div');
    message.innerHTML = `<p>Are you sure you want to <strong>permanently delete</strong> the Sandbox course: <strong>${courseName}</strong> (ID: ${courseID})? This action cannot be undone.</p>`;
    ...
    confirmButton.className = 'buttonmain delete-confirm'; // Potentially for specific styling
    setButtonState(confirmButton, 'Yes, Delete This Sandbox', {isDisabled: false});

    confirmButton.onclick = function () {
      setButtonState(this, 'Deleting...', { isLoading: true, isDisabled: true });
      fetch('https://script.google.com/macros/s/AKfycbxqkbPY18f_CpXY2MRmr2Ou7SVQl5c7HQjnCbaoX0V2621sdC_4N-tPQgeggU0l-QDrFQ/exec', {
        method: 'POST',
        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
        body: new URLSearchParams({
          action: 'SBActions', task: 'delete', courseID, accessToken, userID, userLoginId
        }).toString()
      })
        .then(response => {
        ...
        .then(data => {
          if (data && data.delete === true) {
            processContainer.innerHTML = `<h2>Sandbox Deleted Successfully</h2>
                                         <p>Your Sandbox course "${courseName}" has been deleted. You may now create a new Sandbox course if desired.</p>`;
            const startOverButton = document.createElement('button');
            setButtonState(startOverButton, 'Create New Sandbox / Start Over', {isDisabled: false});
            startOverButton.className = 'buttonmain';
            startOverButton.onclick = handleSandboxSelection; // Navigate to start of sandbox creation
            processContainer.appendChild(startOverButton);
          } else { // Deletion not confirmed or error in data structure
            processContainer.innerHTML = `<h2>Error Deleting Sandbox</h2>
                                         <p>${data.error || 'The server did not confirm deletion. The course might still exist.'}</p>`;
            setButtonState(confirmButton, 'Yes, Delete This Sandbox', { isLoading: false, isDisabled: false });
          }
        })
        .catch(error => {
          processContainer.innerHTML = `<h2>Request Failed</h2><p>Could not delete Sandbox: ${error.message}</p>`;
          setButtonState(confirmButton, 'Yes, Delete This Sandbox', { isLoading: false, isDisabled: false });
        });
    };
    ...
    const previousButton = document.createElement('button');
    previousButton.className = 'buttonmain previous';
    previousButton.textContent = 'Go Back (Cancel)';
    previousButton.onclick = function () {
      // Go back to the sandbox management page, re-fetching the list.
      setButtonState(this, 'Loading...', {isLoading: true, isDisabled: true});
      fetch('https://script.google.com/macros/s/AKfycbxqkbPY18f_CpXY2MRmr2Ou7SVQl5c7HQjnCbaoX0V2621sdC_4N-tPQgeggU0l-QDrFQ/exec', {
        method: 'POST',
        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
        body: `action=SBCheck&accessToken=${encodeURIComponent(accessToken)}`
      ...
      .then(response => response.json())
      .then(data => handleSandboxExistsPage(data.sbCourses || [])) // Ensure array even if undefined
      .catch(() => handleSandboxExistsPage([])); // On error, show management page, likely empty
  };
...
/**
 * Displays the result of a sandbox course reset attempt on a dedicated page.
 * This is typically called after `showResetConfirmation`.
 * @param {object} data - The response data from the reset API call.
 *                        Expected: `{id: string, name: string, link: string}` on success, or `{error: string}` on failure.
 */
function showResetResult(data) {
  ...
  } else if (data.id && data.name) { // Basic check for successful response structure
    processContainer.innerHTML = `<h2>Course Reset Successfully!</h2>
                                 <p><strong>Name:</strong> ${data.name}</p>
                                 <p><strong>ID:</strong> ${data.id}</p>
                                 ${data.link ? `<p><a href="${data.link}" target="_blank" rel="noopener noreferrer">View Reset Course in Canvas</a></p>` : ''}`;
  } else { // Unexpected response
    processContainer.innerHTML = `<h2>Unexpected Response After Reset</h2>
                                 <p>The server responded in an unexpected way. The course may or may not have been reset. Please check Canvas or contact support.</p>`;
    console.error('Unexpected server response for reset result:', data);
  }

  const startOverButton = document.createElement('button');
  startOverButton.textContent = 'Start Over / Manage Sandbox';
  startOverButton.className = 'buttonmain';
  startOverButton.onclick = handleSandboxSelection; // Go back to sandbox agreement/check page
  processContainer.appendChild(startOverButton);
}

/**
 * Displays a full-page confirmation dialog before resetting a sandbox course.
 * Similar to `showDeleteConfirmation`, this might be for a workflow separate from table actions.
 * @param {string} courseID - The ID of the course to be reset.
 * @param {string} accessToken - The user's access token.
 * @param {string} courseName - The name of the course for the confirmation message.
 */
function showResetConfirmation(courseID, accessToken, courseName) {
  ...
  const header = document.createElement('h2');
  header.textContent = 'Confirm Reset Sandbox Course';
  processContainer.appendChild(header);

  const message = document.createElement('div');
  message.innerHTML = `<p>Are you sure you want to reset the Sandbox course: <strong>${courseName}</strong> (ID: ${courseID})? This will clear all content but keep the course shell.</p>`;
  processContainer.appendChild(message);
  ...
  const confirmButton = document.createElement('button');
  confirmButton.className = 'buttonmain reset-confirm';
  setButtonState(confirmButton, 'Yes, Reset This Sandbox', {isDisabled: false});

  confirmButton.onclick = function () {
    setButtonState(this, 'Resetting...', { isLoading: true, isDisabled: true });
    fetch('https://script.google.com/macros/s/AKfycbxqkbPY18f_CpXY2MRmr2Ou7SVQl5c7HQjnCbaoX0V2621sdC_4N-tPQgeggU0l-QDrFQ/exec', {
      method: 'POST',
      headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
      body: new URLSearchParams({
        action: 'SBActions', task: 'reset', courseID, accessToken, userID, userLoginId
      }).toString()
    })
    .then(response => {
      if (!response.ok) throw new Error(`Reset API error: ${response.statusText}`);
      return response.json();
    })
    .then(data => {
      showResetResult(data); // Display the result of the reset operation on a new screen
    })
    .catch(error => {
      processContainer.innerHTML = `<h2>Request Failed</h2><p>Could not reset Sandbox: ${error.message}</p>`;
      setButtonState(confirmButton, 'Yes, Reset This Sandbox', { isLoading: false, isDisabled: false });
      // Optionally add a back button here too, or rely on user to use browser back/start over
    });
  };
  ...
  previousButton.onclick = function () {
    setButtonState(this, 'Loading...', {isLoading: true, isDisabled: true});
     fetch('https://script.google.com/macros/s/AKfycbxqkbPY18f_CpXY2MRmr2Ou7SVQl5c7HQjnCbaoX0V2621sdC_4N-tPQgeggU0l-QDrFQ/exec', {
      ...
      .then(response => response.json())
      .then(data => handleSandboxExistsPage(data.sbCourses || []))
      .catch(() => handleSandboxExistsPage([]));
  };
